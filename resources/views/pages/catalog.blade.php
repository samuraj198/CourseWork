...
@section('content')
    <div class="search-block w-3/4 mb-16">
        <form class="relative w-full" action="{{ route('catalog') }}">
            @csrf
            <input value="{{ old('filename', $filename) }}" name="filename" type="text" class="w-full border-[1px] border-solid border-black rounded-lg py-[13px] pl-[15px] pr-[100px] dark:border-white" placeholder="Введите название 3d-модели">
